 ...
 *    - Get setting: await window.api.invoke(IPC_CHANNELS.SETTINGS_GET, 'settingKey')
 *    - Set setting: await window.api.invoke(IPC_CHANNELS.SETTINGS_SET, 'settingKey', value)
 *    - Listen for changes: window.api.on(IPC_CHANNELS.SETTINGS_CHANGED, ({key, value}) => {})
 *
...
import { IPC, IPC_CHANNELS } from '../ipcChannels.js'
...
export class SettingsHandler extends BaseHandler {
  ...
  async handleSetAutoStart(_, enable) {
    return getAutoStartStatus(enable)
  }
